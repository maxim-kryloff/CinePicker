...
class DiscoverSettingsService {
    ...
    public func requestGenres(onComplete callback: @escaping (_: [Genre]?) -> Void) {
    ...
    public func requestDiscoveredMovies(
        request: RequestToDiscoverMovies,
        onComplete callback: @escaping (_: RequestToDiscoverMovies, _: [Movie]?) -> Void
    ) {
